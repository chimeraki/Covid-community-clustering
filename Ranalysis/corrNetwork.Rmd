...
# get data for each country where the casesPerMil exceeds 0.1

countries <- unique(covid$Code)
datList <- lapply(countries, function(x){
	tmp <- filter(covid, Code==x)
	tmp <- tmp[which(tmp$casesPerMil > 0.1)[1]:nrow(tmp),]
...
# filter to a minimum of 20 observations for a given country
datList2 <- datList[which(plyr::laply(datList, nrow) >= 20)]


# format it into a data.frame
datDF <- plyr::ldply(datList2, function(x){
	return(x$casesPerMil[1:20])
})
...
# make a correlation matrix (I was too dumb to think about normalized difference here)
...
diffMat <- matrix(tmp, ncol=191, nrow=191)
...
plot.igraph(corG, vertex.label=NA, 
	...
	edge.width=1,
  layout=as.matrix(locs[,c("centroid.lon", "centroid.lat")]),
  rescale = FALSE)
...
I think I just calculated the normalized difference wrong or something. One big cluster of bullshit.
